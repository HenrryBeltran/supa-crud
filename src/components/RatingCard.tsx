...
const RatingCard: React.FC<{ rating: number }> = ({ rating }) => {
  const fullStar = (
    <svg className="h-3 w-3 fill-yellow-500/80 lg:h-5 lg:w-5" viewBox="0 96 960 960">
  ...
  const emptyStar = (
    <svg className="h-3 w-3 fill-slate-400/80 lg:h-5 lg:w-5" viewBox="0 96 960 960">
  ...
  function renderStarCollection(amount: number, star: React.JSX.Element) {
    ...
    return array.map((star, index) => <div key={index}>{star}</div>);
  }
